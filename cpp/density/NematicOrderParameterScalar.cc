...
#include "NematicOrderParameterScalar.h"
...
NematicOrderParameterScalar::NematicOrderParameterScalar(float rmax, float dr)
...
NematicOrderParameterScalar::~NematicOrderParameterScalar()
    {
    for (tbb::enumerable_thread_specific<unsigned int *>::iterator i = m_local_bin_counts.begin(); i != m_local_bin_counts.end(); ++i)
        {
        delete[] (*i);
        }
    for (typename tbb::enumerable_thread_specific<float *>::iterator i = m_local_rdf_array.begin(); i != m_local_rdf_array.end(); ++i)
        {
        delete[] (*i);
        }
    }
...
void NematicOrderParameterScalar::reduceCorrelationFunction()
    {
    memset((void*)m_bin_counts.get(), 0, sizeof(unsigned int)*m_nbins);
    for(size_t i(0); i < m_nbins; ++i)
        m_rdf_array.get()[i] = float();
    ...
    parallel_for(tbb::blocked_range<size_t>(0, m_nbins),
        ...
        for (size_t i = r.begin(); i != r.end(); i++)
            ...
            for (typename tbb::enumerable_thread_specific<float *>::const_iterator local_rdf = m_local_rdf_array.begin();
...
std::shared_ptr<float> NematicOrderParameterScalar::getRDF()
    {
    if (m_reduce == true)
        {
...
*/
void NematicOrderParameterScalar::reset()
    {
    ...
    for (typename tbb::enumerable_thread_specific<float *>::iterator i = m_local_rdf_array.begin(); i != m_local_rdf_array.end(); ++i)
        {
        memset((void*)(*i), 0, sizeof(float)*m_nbins);
        }
...
void NematicOrderParameterScalar::accumulate(const box::Box &box,
                             const freud::locality::NeighborList *nlist,
                             const vec3<float> *ref_points,
                             const vec3<float> *ref_values,
                             unsigned int n_ref,
                             const vec3<float> *points,
                             const vec3<float> *point_values,
                             unsigned int Np)
    {
    m_box = box;
    ...
    parallel_for(tbb::blocked_range<size_t>(0, n_ref),
        ...
        if (!rdf_exists)
            {
            m_local_rdf_array.local() = new float[m_nbins];
            memset((void*)m_local_rdf_array.local(), 0, sizeof(float)*m_nbins);
            }
        ...
        for (size_t i = r.begin(); i != r.end(); i++)
            ...
            for(; bond < nlist->getNumBonds() && neighbor_list[2*bond] == i; ++bond)
			    ...
			    float dotOrientations = dot(ref_values[i],point_values[j]);
                            m_local_rdf_array.local()[bin] += 2.0*dotOrientations*dotOrientations-1.0;
                            }
                        }
                    }
                }
