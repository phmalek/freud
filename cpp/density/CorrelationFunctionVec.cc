...
#include "CorrelationFunctionVec.h"
...
CorrelationFunctionVec::CorrelationFunctionVec(float rmax, float dr)
...
CorrelationFunctionVec::~CorrelationFunctionVec()
    {
...
void CorrelationFunctionVec::reduceCorrelationFunction()
    {
    memset((void*)m_bin_counts.get(), 0, sizeof(unsigned int)*m_nbins);
    for(size_t i(0); i < m_nbins; ++i)
        m_rdf_array.get()[i] = float();
...
std::shared_ptr<float> CorrelationFunctionVec::getRDF()
    {
    if (m_reduce == true)
        {
...
*/
void CorrelationFunctionVec::reset()
    {
...
void CorrelationFunctionVec::accumulate(const box::Box &box,
                ...
                const size_t j(neighbor_list[2*bond + 1]);
                    ...
                    if ((i != j || points != ref_points) && rsq < rmaxsq)
                        ...
                        if (bin < m_nbins)
                            ...
                            m_local_rdf_array.local()[bin] += dot(ref_values[i],point_values[j]);
                            }
                        }
                    }
                }
